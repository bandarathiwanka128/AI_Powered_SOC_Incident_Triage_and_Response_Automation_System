...
Pipeline:
  Stage 1 — PreFilter     : fast, high-confidence pattern matching
  Stage 3 — ExpertSystem  : broader rule-based fallback
  (Stage 2 — MLPredictor sits between them in app.py / api_server.py)
"""
...
class PreFilter:
    """
    Stage 1: Fast deterministic rules for unmistakable, high-volume attacks.
    Runs BEFORE the ANN model.  If a rule fires we skip ML entirely.

    Thresholds are deliberately strict so only obvious cases are caught here;
    ambiguous traffic falls through to the ANN (Stage 2) or Expert System (Stage 3).
    """
